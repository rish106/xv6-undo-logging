...
{
  read_head();
  recover_install_trans(); // if committed, copy from log to disk
  log.lh.n = 0;
...
void
write_log(struct buf *b) {
  struct buf *lbuf = bread(log.dev, log.start + 1); // read log block
  memmove(lbuf->data, b->data, BSIZE);
  bwrite(lbuf);  // write the log
  brelse(b);
  brelse(lbuf);
}
